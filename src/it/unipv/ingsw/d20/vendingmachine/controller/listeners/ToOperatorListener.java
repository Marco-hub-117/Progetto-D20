...
/**
 * Listener del tasto che permette di passare alla modalit‡ operatore: alla pressione,
 * prima viene richiesta la chiave della macchinetta, dopodichÈ, se giusta, si passa
 * alla modalit‡ operatore vera e propria.
 *
 */
public class ToOperatorListener implements ActionListener {
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		opLogin.getEnterButton().addActionListener(al -> { //aggiunge il listener al bottone del frame appena aperto
			if (vm.isCorrectId(opLogin.getInsertedKey())) { //controlla che la chiave inserita corrisponda all'id della macchina
				opLogin.dispose();
				openOperatorGui(); //se √® vero entra in modalit√† operatore
			} else {
	...
	/**
	 * Metodo che esegue effettivamente il passaggio alla modalit‡ operatore, dopo che Ë
