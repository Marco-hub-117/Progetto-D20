package it.unipv.ingsw.d20.util;

/**
 * Classe che contiene tutti i percorsi utili ai file.
 *
 */
public class Paths {
	
	private Paths() {}
	
	public static final String ASSETS_FOLDER = "assets/";
	
	public static final String RES_FOLDER = "res/";
	
	
	public static final String IMG_FOLDER = "img/";
	
	public static final String FONT_FOLDER = "font/";
	
	
	public static final String BEVERAGE_CATALOG = "beverageCatalog";
	
	public static final String VENDING_ID = "vendingID";
	
	public static final String TANKS = "tanks";
	
	public static final String CASH_CONTAINER_STATUS = "cashContainerStatus";

	public static final String VENDING_TYPE = "vendingType";

	public static final String SALE_LIST = "saleList";
	
	
	public static final String GEAR_ICON = "gearicon.png";
	
	public static final String JAVA_IMG = "java.png";
	
	public static final String DISPLAY_FONT = "DS-DIGIB.TTF";
	
}
